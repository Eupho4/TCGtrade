...
class AsianMigrationFull {
    constructor() {
        this.db = new LocalCardDatabase();
        this.asianLanguages = ['ja', 'zh-cn', 'zh-tw']; // Removido 'ko' temporalmente
        this.totalCards = 0;
        this.totalSets = 0;
        this.batchSize = 20;
        this.maxSetsPerLanguage = 10; // Aumentar para migrar más sets
    }

    async init() {
        console.log('🗄️ Inicializando migración asiática completa...');
        await this.db.init();
        ...
        for (const lang of this.asianLanguages) {
            try {
                ...
                const validSets = sets.filter(set => 
                    ...
                    !set.id.includes('pocket') && 
                    set.cardCount?.total > 0
                ).slice(0, this.maxSetsPerLanguage); // Migrar más sets
                ...
                for (const set of validSets) {
                    try {
                        ...
                        await this.delay(1000); // 1 segundo entre sets
    ...
    async getMigrationStats() {
        try {
            ...
            // Mostrar info sobre coreano
            console.log(`- Cartas en KO: 0 (TCGDEX no tiene datos de cartas coreanas aún)`);
    ...
    async start() {
        try {
            ...
            console.log('🚀 Iniciando migración asiática completa...');
            console.log(`📋 Idiomas objetivo: ${this.asianLanguages.join(', ')}`);
            console.log(`⚠️ Nota: Coreano (KO) no disponible en TCGDEX aún`);
            console.log(`📦 Migrando hasta ${this.maxSetsPerLanguage} sets por idioma`);
...
if (require.main === module) {
    const migration = new AsianMigrationFull();
    migration.start().catch(console.error);
}

module.exports = AsianMigrationFull;