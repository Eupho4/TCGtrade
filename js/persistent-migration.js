...
class PersistentMigration {
    constructor() {
        ...
        this.isRunning = true;
        this.maxRetries = 20; // Máximo de reintentos
        this.timeoutMs = 120000; // 2 minutos de timeout
        this.currentPage = 65; // Empezar desde página 65
        this.requestDelay = 5000; // 5 segundos entre requests
    }

    async init() {
        console.log('🗄️ Inicializando migración PERSISTENTE...');
        await this.db.init();
        await this.searchEngine.init();
        console.log('✅ Migración persistente inicializada');
    }

    async migratePersistently() {
        try {
            console.log('🚀 Iniciando MIGRACIÓN PERSISTENTE de la API de Pokémon TCG...');
            console.log('🛡️ Configurado para ser EXTREMADAMENTE persistente');
            console.log('⚡ Timeouts: 2 minutos, Reintentos: 20, Delay: 5s entre requests');
            this.migrationStats.startTime = new Date();
            
            // 1. Migrar TODOS los sets primero
            console.log('\n📚 FASE 1: Migrando TODOS los sets...');
            await this.migrateAllSetsPersistently();
            
            // 2. Migrar TODAS las cartas restantes
            console.log('\n🎴 FASE 2: Migrando TODAS las cartas restantes...');
            await this.migrateAllCardsPersistently();
            ...
            console.log('🎉 MIGRACIÓN PERSISTENTE COMPLETADA');
            
        } catch (error) {
            console.error('❌ Error en migración persistente:', error);
            this.printProgress();
            throw error;
        }
    }

    async migrateAllSetsPersistently() {
        try {
            console.log('🌐 Obteniendo TODOS los sets con manejo PERSISTENTE...');
            ...
            while (hasMore && this.isRunning && consecutiveErrors < maxConsecutiveErrors) {
                try {
                    ...
                    const response = await this.fetchWithPersistentRetry(url);
    ...
    async migrateAllCardsPersistently() {
        try {
            console.log('🌐 Obteniendo TODAS las cartas con manejo PERSISTENTE...');
            console.log(`📄 Empezando desde página ${this.currentPage}...`);
            
            let page = this.currentPage;
            let hasMore = true;
            let totalCards = 0;
            let batchSize = 10; // Lotes ultra pequeños
            let consecutiveErrors = 0;
            const maxConsecutiveErrors = 15;
                ...
                try {
                    ...
                    if (data.data && data.data.length > 0) {
                        ...
                        for (let i = 0; i < data.data.length; i += batchSize) {
                            ...
                            await this.processCardBatchPersistently(batch);
                    ...
                    if (consecutiveErrors < maxConsecutiveErrors) {
                        const waitTime = Math.min(120000 * consecutiveErrors, 1200000); // Hasta 20 minutos
                        console.log(`⏳ Esperando ${waitTime/1000} segundos antes del siguiente intento...`);
                        await this.delay(waitTime);
    ...
    async processCardBatchPersistently(cards) {
        try {
            for (const card of cards) {
                if (!this.isRunning) break;
                await this.saveCardPersistently(card);
            }
        } catch (error) {
    ...
    async saveCardPersistently(card) {
        try {
            const cardData = {
                id: card.id,
                name: card.name,
    ...
    async fetchWithPersistentRetry(url, maxRetries = null) {
        ...
        for (let attempt = 1; attempt <= retries; attempt++) {
            try {
                ...
                const headers = {
                    'User-Agent': 'TCGtrade-Persistent-Migration/1.0',
                ...
                const delay = Math.min(10000 * Math.pow(2, attempt - 1), 300000); // Hasta 5 minutos
    ...
    printProgress() {
        ...
        console.log(`🌐 Requests totales: ${this.migrationStats.totalRequests}`);
    ...
    printFinalStats() {
        ...
        console.log('\n🎉 MIGRACIÓN PERSISTENTE COMPLETADA');
        console.log('=====================================');
        console.log(`📚 Sets migrados: ${this.migrationStats.setsMigrated}`);
        console.log(`🎴 Cartas migradas: ${this.migrationStats.cardsMigrated}`);
        console.log(`❌ Errores: ${this.migrationStats.errors}`);
...
module.exports = PersistentMigration;

// Si se ejecuta directamente, iniciar migración
if (require.main === module) {
    const migrator = new PersistentMigration();
    ...
    migrator.init()
        .then(() => migrator.migratePersistently())
        .then(() => migrator.close())
        .catch(error => {
            console.error('💥 Error fatal en migración persistente:', error);
            migrator.printProgress();
            process.exit(1);
        });
}