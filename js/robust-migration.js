...
class RobustMigration {
    constructor() {
        ...
        this.isRunning = true;
        this.maxRetries = 10; // Más reintentos
        this.timeoutMs = 60000; // 60 segundos de timeout
    }

    async init() {
        console.log('🗄️ Inicializando migración robusta...');
        await this.db.init();
        await this.searchEngine.init();
        console.log('✅ Migración robusta inicializada');
    }

    async migrateRobustly() {
        try {
            console.log('🚀 Iniciando MIGRACIÓN ROBUSTA de la API de Pokémon TCG...');
            console.log('🛡️ Configurado para manejar timeouts y errores persistentemente');
            this.migrationStats.startTime = new Date();
            
            // 1. Migrar TODOS los sets primero
            console.log('\n📚 FASE 1: Migrando TODOS los sets...');
            await this.migrateAllSetsRobustly();
            
            // 2. Migrar TODAS las cartas restantes
            console.log('\n🎴 FASE 2: Migrando TODAS las cartas restantes...');
            await this.migrateAllCardsRobustly();
            ...
            console.error('❌ Error en migración robusta:', error);
            this.printProgress();
            throw error;
        }
    }

    async migrateAllSetsRobustly() {
        try {
            console.log('🌐 Obteniendo TODOS los sets con manejo robusto de errores...');
            ...
            while (hasMore && this.isRunning && consecutiveErrors < maxConsecutiveErrors) {
                try {
                    ...
                    const response = await this.fetchWithRobustRetry(url);
    ...
    async migrateAllCardsRobustly() {
        try {
            console.log('🌐 Obteniendo TODAS las cartas con manejo robusto de errores...');
            
            // Empezar desde la página 39 (aproximadamente donde se quedó con 9,528 cartas)
            let page = 39;
            let hasMore = true;
            let totalCards = 0;
            let batchSize = 20; // Lotes muy pequeños
            let consecutiveErrors = 0;
                ...
                try {
                    ...
                    if (data.data && data.data.length > 0) {
                        ...
                        for (let i = 0; i < data.data.length; i += batchSize) {
                            ...
                            await this.processCardBatchRobustly(batch);
                            totalCards += batch.length;
                            
                            const progress = ((page - 39) / 40) * 100; // Estimación de 40 páginas más
                    ...
                    if (consecutiveErrors < maxConsecutiveErrors) {
                        const waitTime = Math.min(15000 * consecutiveErrors, 60000); // Hasta 1 minuto
    ...
    async processCardBatchRobustly(cards) {
        try {
            for (const card of cards) {
                if (!this.isRunning) break;
                await this.saveCardRobustly(card);
            }
        } catch (error) {
    ...
    async fetchWithRobustRetry(url, maxRetries = null) {
...
if (require.main === module) {
    const migrator = new RobustMigration();
    ...
    migrator.init()
        .then(() => migrator.migrateRobustly())
        .then(() => migrator.close())
        .catch(error => {
            console.error('💥 Error fatal en migración robusta:', error);
            migrator.printProgress();
            process.exit(1);
        });
}